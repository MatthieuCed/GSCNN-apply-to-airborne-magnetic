...
def prep_experiment(args, parser):
    ...
    args.exp_path = '{0}results/'.format(cfg.DATASET.CITYSCAPES_DIR)
    
    args.tb_exp_path = os.path.join(tb_path, args.exp, exp_name)
    args.ngpu = torch.cuda.device_count()
    
    #je fais un petit changement là 
    if args.ngpu ==0:
        args.ngpu=1
...
def evaluate_eval(args, net, optimizer, val_loss, mf_score, hist, dump_images, heatmap_images, writer, epoch=0, dataset=None, ):
    ...
    #print_evaluate_results(hist, iu, writer, epoch, dataset) #on enlève ça car ça fait des errueurs
    ...
    print('mean_iu best record : ', args.best_record['mean_iu'])
    
    # update best snapshot
    if mean_iu > args.best_record['mean_iu'] : #< for >
        ...
        for bs_idx, bs_data in enumerate(dump_images):
            for local_idx, data in enumerate(zip(bs_data[0], bs_data[1],bs_data[2],bs_data[3])):
                ...
                gt_pil = data[0].cpu().numpy()
                gt_fn = '{}_seg_gt.png'.format(img_name)
                plt.imsave(os.path.join(to_save_dir, gt_fn), gt_pil)
                #print('path_to_save : ', os.path.join(to_save_dir, gt_fn))
                
                pred = data[1].cpu().numpy()
                prediction_fn = '{}_seg_prediction.png'.format(img_name)
                plt.imsave(os.path.join(to_save_dir, prediction_fn), pred)

                mag_true = data[3].cpu().numpy()
                mag_true = np.moveaxis(mag_true, 0, -1)
                mag_true = (mag_true - np.min(mag_true))/(np.max(mag_true)-np.min(mag_true))
                prediction_fn = '{}_truth.png'.format(img_name)
                plt.imsave(os.path.join(to_save_dir, prediction_fn), mag_true)                
                
                gt_pil = args.dataset_cls.colorize_mask(gt_pil)
        ...
        for bs_idx, bs_data in enumerate(heatmap_images):
            for local_idx, data in enumerate(zip(bs_data[0], bs_data[1],bs_data[2])):
                ...
                gt_pil_ed = data[0].cpu().int().numpy()
                gt_fn = '{}_edge_gt.png'.format(img_name)
                plt.imsave(os.path.join(to_save_dir, gt_fn), gt_pil_ed)
                
                predictions_pil = data[1].cpu().int().numpy()
                prediction_fn = '{}_edge_prediction.png'.format(img_name)
                plt.imsave(os.path.join(to_save_dir, prediction_fn), predictions_pil)
                                                
                ip.add_table([(gt_fn, 'gt'), (prediction_fn, 'prediction')])
                
                predictions_pil = (predictions_pil / predictions_pil.max()) 
                predictions_pil = Image.fromarray(predictions_pil.astype(np.uint8))
                                
                gt_pil_ed = args.dataset_cls.colorize_mask(gt_pil_ed)
                
                # val_visual.extend([visualize(gt_pil_ed.convert('RGB')),
                #                     visualize(predictions_pil.convert('RGB'))])
                idx = idx+1
                if idx >= 9:
                    ip.write_page()
    ...
    logging.info(fmt_str % (args.best_record['val_loss'], 
